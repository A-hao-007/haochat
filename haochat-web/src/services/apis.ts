...
export default {
  ...
  /** 免打扰 */
  muteContact: (roomId: number, muted: boolean) =>
    putRequest<void>(urls.muteContact, null, { params: { roomId, muted } }),
  /** 删除会话 */
  deleteContact: (roomId: number) =>
    deleteRequest<void>(urls.deleteContact, null, { params: { roomId } }),
  /** 设置好友备注 */
  setFriendRemark: (friendUid: number, remark: string) =>
    putRequest<void>(urls.setFriendRemark, null, { params: { friendUid, remark } }),
  /** 上传文件 */
