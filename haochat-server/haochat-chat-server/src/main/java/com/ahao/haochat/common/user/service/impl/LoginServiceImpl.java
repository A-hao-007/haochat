...
public class LoginServiceImpl implements LoginService {
    ...
    public String login(Long uid) {
        ...
        try {
            ...
            log.warn("Ignore unreadable login token for uid={}", uid, e);
        }
        if (StrUtil.isNotBlank(token)) {
            Long tokenUid = jwtUtils.getUidOrNull(token);
            if (Objects.equals(uid, tokenUid)) {
                return token;
            }
            log.warn("Refresh invalid login token for uid={}", uid);
        }
        //获取用户token
